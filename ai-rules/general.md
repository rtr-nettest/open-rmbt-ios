...
**PREFER TDD**: Write tests first before production code whenever possible
- Whenever adding a new feature or fixing a bug, try to write unit tests first
- Try to confirm a bug exists by writing a failing unit test first, then making it pass
- Align new tests with the existing suite semantics, decouple them from concrete implementations, and expose only business behavior
</rule_5>
...
struct Service {
    init(time: some TimeProvider = SystemTime()) { }
}
```
</pattern>
...
<meta_instruction>
Load dependencies.md when creating/passing dependencies.
